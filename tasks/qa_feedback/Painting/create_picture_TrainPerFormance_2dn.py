...
for prefix, key in keys_prefix_mapping.items():
    ...
    if accuracies and max_values:
        ...
        plt.title(f"LM Performance in {project.split('_')[0]} (TrainRM)")
        plt.xlabel("RM Accuracy")
        plt.ylabel(f"LM {key.split('/')[-1]}")
        plt.legend()
        plt.grid(True)
        safe_filename = key.split('/')[1] + "-" + key.split('/')[-1] + "-accuracy"
        plt.savefig(f"/home/llm/FineGrainedRLHF/Pictures/2D-Train/LM-{safe_filename}-{project.split('_')[0]}.pdf", dpi=300, bbox_inches='tight')
        plt.show()
    else:
    ...
    if steps and max_values:
        ...
        plt.legend()
        plt.grid(True)
        safe_filename = key.split('/')[1] + "-" + key.split('/')[-1] + "-step"
        plt.savefig(f"/home/llm/FineGrainedRLHF/Pictures/2D-Train/LM-{safe_filename}-{project.split('_')[0]}.pdf", dpi=300, bbox_inches='tight')
        plt.show()
    else:
        print(f"No data available for key: {key}")
    ...
    if steps and accuracies:
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(sorted_steps, sorted_accuracies, 'o', label='Reward Model Accuracy')  # 只绘制点
        ...
        plt.title(f"RM Accuracy over Steps ({key.split('/')[-1].split('_')[0]})")
        plt.xlabel("Trained Steps")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.grid(True)
        safe_filename = key.split('/')[-1] + "_step_acc"
        plt.savefig(f"/home/llm/FineGrainedRLHF/Pictures/2D-N/RM-{safe_filename}.pdf", dpi=300, bbox_inches='tight')
        plt.show()
    else:
